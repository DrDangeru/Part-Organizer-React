...
// Location operations
export const insertLocation = (locationName: string, container: string, row: string, position: string): Promise<Location> => {
  return new Promise((resolve, reject) => {
    db.run(
      ...
      function(err) {
        if (err) reject(err);
        resolve({ id: this.lastID, locationName, container, row, position } as Location);
      }
    );
  });
};
...
// Part operations
export const insertPart = (partName: string, partDetails: string, 
  locationName: string, container: string, row: string, position: string): Promise<Part> => {
  return new Promise((resolve, reject) => {
    console.log('Attempting to insert part:', { partName, partDetails, locationName, container, row, position });
    
    // First check if the location exists
    db.get('SELECT locationName FROM locations WHERE locationName = ?', [locationName], (err, location) => {
      if (err) {
      ...
      db.run(
        ...
        function(err) {
          ...
          const part = { id: this.lastID, partName, partDetails, locationName, container, row, position } as Part;
